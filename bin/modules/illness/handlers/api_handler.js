...
const listIllness = async (req, res) => {
    const ctx = 'apiHandler-listIllness';
    const { query } = req;
    ...
    const { data } = isValid;
    const result = await queryHandler.listIllness({ ...data });
...
const getIllness = async (req, res) => {
    ...
    const isValid = await validator.isValidPayload(payload, queryModel.getIllness());
    if (isValid.err) {
        return sendResponse(isValid, res);
    }
    const { data } = isValid;
    const result = await queryHandler.getIllness({ ...data });
    return paginationResponse(result, res, 'Hooray, This is Illness you\'er asking for');
};
